...
class ESPLoader(object):
    ...
    def _wait_for_json(self, timeout = 1):
        json_msg = self._read_json()
        init_time = time.time()
        while not json_msg:
            json_msg = self._read_json()
            time.sleep(0.1)
    ...
    def get_network_uuid(self):
        ...
        while True:
            ...
            try:
                json_msg = json.loads(self._wait_for_json())
    ...
    def get_esp_version(self):
        ...
        ver = base64.b64decode(json_msg["b"]).lstrip(b"\x00")
        return ver.decode("ascii")

    def set_esp_version(self, version_text: str, retry = 5):
        ...
        version_text = base64.b64encode(version_byte).decode("utf8")
        version_msg = (
            "{" + f'"c":160,"s":24,"d":4095,'
    ...
    def _connect_attempt(self, mode='default_reset', esp32r0_delay=False):
        ...
        self.esp_version = self.get_esp_version()
        if self.esp_version is not None:
            print("esp version", self.esp_version)
    ...
    if args.flash_size != 'keep':  # TODO: check this even with 'keep'
        ...
        for address, argfile in args.addr_filename:
            ...
            if address + argfile.tell() > flash_end:
                raise FatalError(("File %s (length %d) at offset %d will not fit in %d bytes of flash. Use --flash-size argument, or change flashing address.") % (argfile.name, argfile.tell(), address, flash_end))
            argfile.seek(0)
